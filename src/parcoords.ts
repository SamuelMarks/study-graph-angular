...
export function d3parcoords(config) {
  const __ = {
    data: [],
    highlighted: [],
    dimensions: {},
  ...
  const pc: any = function(selection) {
  ...
  const eventTypes = ['highlight', 'brush', 'brushend', 'brushstart', 'axesreorder'].concat(d3.keys(__));

  const events = d3.dispatch(...eventTypes);

  const render = d3.dispatch('render');
  ...
  const resize = d3.dispatch('resize');
  ...
  const w = function() { return __.width - __.margin.right - __.margin.left; },
...
// side effects for setters
  ...
  const bundleDimension = d3.dispatch('bundleDimension');
  ...
  const hideAxis = d3.dispatch('hideAxis');
  hideAxis.on('hideAxis', function(d) {
    pc.dimensions(pc.applyDimensionDefaults());
    pc.dimensions(without(__.dimensions, d.value));
  });
  const flipAxes = d3.dispatch('flipAxes');
...
// getter/setter with event firing
  ...
  pc.autoscale = function() {
    ...
    const defaultScales = {
      'date': function(k) {
        let extent = d3.extent(__.data, function(d) {
      ...
      'number': function(k) {
        let extent = d3.extent(__.data, function(d) { return +d[k]; });
        // special case if single value
        if (extent[0] === extent[1]) {
          return d3.scaleOrdinal()
            .domain([extent[0]])
      ...
      'string': function(k) {
        ...
        if (__.flipAxes.includes(k)) {
          ...
          const tempArr = Object.getOwnPropertyNames(counts).sort();
        ...
        const addBy = getRange()[0] / (domain.length - 1);
...
// draw dots with radius r on the axis line where data intersects
  pc.axisDots = function(r) {
    r = r || 0.1;
    const _ctx = pc.ctx.marks;
    const startAngle = 0;
    const endAngle = 2 * Math.PI;
    _ctx.globalAlpha = d3.min([1 / Math.pow(__.data.length, 1 / 2), 1]);
    __.data.forEach(function(d) {
      d3.entries(__.dimensions).forEach(function(p, i) {
        _ctx.beginPath();
        _ctx.arc(position(p), __.dimensions[p.key].yscale(d[p]), r, startAngle, endAngle);
        _ctx.stroke();
        _ctx.fill();
      });
    });
...
// draw single cubic bezier curve
  function single_curve(d, _ctx) {

    const centroids = compute_centroids(d);
    const cps = compute_control_points(centroids);

    _ctx.moveTo(cps[0].e(1), cps[0].e(2));
    for (let i = 1; i < cps.length; i += 3) {
      if (__.showControlPoints) {
        for (let j = 0; j < 3; j++) {
          _ctx.fillRect(cps[i + j].e(1), cps[i + j].e(2), 2, 2);
        }
      }
      _ctx.bezierCurveTo(cps[i].e(1), cps[i].e(2), cps[i + 1].e(1), cps[i + 1].e(2), cps[i + 2].e(1), cps[i + 2].e(2));
    }
  };

// draw single polyline
  function color_path(d, _ctx) {
    _ctx.beginPath();
...
// draw many polylines of the same color
  function paths(data, _ctx) {
    _ctx.clearRect(-1, -1, w() + 2, h() + 2);
    _ctx.beginPath();
...
// returns the y-position just beyond the separating null value line
  ...
  function single_path(d, _ctx) {
    d3.entries(__.dimensions).forEach(function(p, i) {  //p isn't really p
      if (i == 0) {
        _ctx.moveTo(position(p.key), typeof d[p.key] == 'undefined' ? getNullPosition() : __.dimensions[p.key].yscale(d[p.key]));
      } else {
        _ctx.lineTo(position(p.key), typeof d[p.key] == 'undefined' ? getNullPosition() : __.dimensions[p.key].yscale(d[p.key]));
      }
    });
  };
          ...
          d3.select(this).call(
            __.dimensions[d]['brush']
              ...
              .on('end', function() {
                ...
                if (d3.event.sourceEvent.ctrlKey) {
                  let html = d3.select(this).select('.selection').nodes()[0].outerHTML;
                  html = html.replace('class="selection"', ('class="selection dummy' + ' selection-' + __.brushes.length + '"'));
                  const dat = d3.select(this).nodes()[0].__data__;
                  const brush = {
  ...
  pc.brushReset = function(dimension) {
    ...
    if (g) {
      const nodes = d3.selectAll('.brush').nodes();
    ...
    if (__.brushes.length === 0) {
      ...
      for (let k = 0; k < nodes.length; k++) {
        if (d3.brushSelection(nodes[k]) !== null) {
          ...
          const values = [];
          const ranger = d3.brushSelection(nodes[k]);
          if (typeof __.dimensions[nodes[k].__data__].yscale.domain()[0] === 'number') {
            ...
            const dimRange = __.dimensions[nodes[k].__data__].yscale.range();
            const dimDomain = __.dimensions[nodes[k].__data__].yscale.domain();
      ...
      const within = {
        'date': function(d, p, dimension) {
          const category = d[p];
          const categoryIndex = __.dimensions[p].yscale.domain().indexOf(category);
          const categoryRangeValue = __.dimensions[p].yscale.range()[categoryIndex];
      ...
      const multiBrushData = [];
      for (let idx = 0; idx < __.brushes.length; idx++) {
        const brush = __.brushes[idx];
        const values = [];
        const ranger = brush.extent;
        ...
        if (typeof __.dimensions[brush.data].yscale.domain()[0] === 'number') {
          ...
          const dimRange = __.dimensions[brush.data].yscale.range();
          const dimDomain = __.dimensions[brush.data].yscale.domain();
          for (let j = 0; j < dimRange.length; j++) {
...
d3.renderQueue = (function(func) {
  ...
  const rq: {
    invalidate?: () => void, render?: () => void, clear?: (o) => void
    data?: (d) => void, remaining?: () => void, rate?: (v) => void
  } = function(data) {
